...
#NFT #Base #Clawcrab`
  ...
  // Load OpenSea stats for top bar
  useEffect(() => {
    const load = async () => {
      const res = await fetch('/api/opensea-stats');
      const data = await res.json()
      setMarketStats(data)
    }
  ...
  // Load OpenSea activity + daily change feed for left activity box
  useEffect(() => {
    const tick = async () => {
...
const loadNFTImage = async (tokenId: string) => {
  ...
  // ---- Top bar values from OpenSea (safe fallback)
  const os = marketStats?.stats ?? marketStats // depends on your route response shape
  const floor = os?.floor_price
  const oneDayVol = os?.intervals[0]?.volume
  const oneDaySales = os?.intervals[0]?.sales
  const totalVol = os?.total_volume
  const marketCap = os?.market_cap
  const owners = os?.num_owners
  const listed = os?.num_listings
...
text-red-200">
		...
		{/* github */}
  <a
    href="https://github.com/shahrokh11"
    target="_blank"
    className="opacity-70 hover:opacity-100 transition"
  >
    <img src="/icons/github.png" className="w-7 h-7" />
  </a>

  {/* X / Twitter */}
  <a
    href="https://x.com/clawcrab_ai"
    target="_blank"
    className="opacity-70 hover:opacity-100 transition"
  >
  ...
  {/* OpenSea */}
  <a
    href="https://opensea.io/collection/clawcrabnft"
    target="_blank"
    className="opacity-70 hover:opacity-100 transition"
  >
    <img src="/icons/opensea.png" className="w-7 h-7" />
  </a>

  {/* Magic Eden */}
  <a
    href="https://magiceden.io/collections/base/0xf88f805f8281af3b37f0547a899320499591dfe0"
    target="_blank"
    className="opacity-70 hover:opacity-100 transition"
  >
...
<p className="text-zinc-400 max-w-xl leading-relaxed space-y-2 flex flex-col">
  <span>Once upon a block, a colony of Claw Crabs emerged on Base.</span>
...
bg-[radial-gradient(circle_at_center,_#8F1D2C,_#130308)]">
  ...
  <div className="max-w-9xl mx-auto px-6">   
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>Clawcrab NFT — Built on Base</div>
